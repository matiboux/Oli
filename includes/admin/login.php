	...
	/** Login */
		...
		if(empty($_['username'] = trim($_['username']))) $resultCode = 'E:Please enter your username or your email.';
			...
			if(!$isExistByUsername AND ($isLocalLogin OR !$isExistByEmail)) $resultCode = 'E:Sorry, no account is associated with the username or email you entered.';
				...
				if($_Oli->isAccountsManagementReady()) $_Oli->insertAccountLine('LOG_LIMITS', array('id' => $_Oli->getLastAccountInfo('LOG_LIMITS', 'id') + 1, 'username' => $_['username'], 'user_id' => $_Oli->getUserID(), 'ip_address' => $_Oli->getUserIP(), 'action' => 'login', 'last_trigger' => date('Y-m-d H:i:s')));
